### BASIC SQL
/*
A query to grab three specific columns from the web_event
and only the first 15 rows
*/
SELECT occurred_at, account_id, channel
FROM web_events
LIMIT 15;

/* Top 5 orders in $$$ in the table */
SELECT id, account_id, total_amt_usd
FROM orders
ORDER BY total_amt_usd DESC
LIMIT 5;

/* Sorted by customer, then by dollar amount */
SELECT id, account_id, total_amt_usd
FROM orders
ORDER BY account_id, total_amt_usd DESC;

/* Sorted by $$$, then by customer ID (which is redundant) */
SELECT id, account_id, total_amt_usd
FROM orders
...
SELECT id, account_id, poster_amt_usd / total_amt_usd AS poster_percent
FROM orders
