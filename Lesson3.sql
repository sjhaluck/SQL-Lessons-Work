...
SELECT SUM(poster_qty) total_poster_quantity
FROM orders;

-- Find the total amount of standard_qty paper ordered in the orders table.
SELECT SUM(standard_qty) total_standard_quantity
FROM orders;

-- Find the total dollar amount of sales using the total_amt_usd in the orders table.
SELECT SUM(total_amt_usd) total_money
FROM orders;

-- Find the total amount spent on standard_amt_usd and gloss_amt_usd paper for each order in the orders table. This should give a dollar amount for each order in the table.
SELECT standard_amt_usd+gloss_amt_usd std_gloss_money
FROM orders;

-- Find the standard_amt_usd per unit of standard_qty paper. Your solution should use both an aggregation and a mathematical operator.
SELECT SUM(standard_amt_usd)/SUM(standard_qty) average_std_unit_price
FROM orders;
...
SELECT a.name, MIN(o.occurred_at) earliest_order
FROM orders o
JOIN accounts a
...
SELECT a.name, SUM(o.total_amt_usd) total_value
FROM orders o
JOIN accounts a
ON o.account_id = a.id
GROUP BY a.name
...
SELECT MIN(we.occurred_at) earliest_event, a.primary_poc
FROM web_events we
JOIN accounts a
...
SELECT r.name, COUNT(s.id) total_reps
FROM sales_reps s
JOIN region r
ON s.region_id = r.id
GROUP BY r.name
...
SELECT a.name, AVG(o.standard_amt_usd) std_avg_usd, AVG(o.gloss_amt_usd) gloss_avg_usd, AVG(o.poster_amt_usd) poster_avg_usd
FROM orders o
JOIN accounts a
ON o.account_id = a.id
GROUP BY a.name
...
SELECT s.name, we.channel, COUNT(we.id)
FROM web_events we
JOIN accounts a
ON we.account_id = a.id
JOIN sales_reps s
ON a.sales_rep_id = s.id
GROUP BY s.name, we.channel
ORDER BY s.name, count DESC;

-- Determine the number of times a particular channel was used in the web_events table for each region. Your final table should have three columns - the region name, the channel, and the number of occurrences. Order your table with the highest number of occurrences first.
SELECT r.name, we.channel, COUNT(we.id)
FROM web_events we
JOIN accounts a
ON we.account_id = a.id
JOIN sales_reps s
...
SELECT DATE_PART('year',occurred_at), SUM(total) total_qty
FROM orders
GROUP BY 1
ORDER BY 2 DESC;

-- Which month did Parch & Posey have the greatest sales in terms of total number of orders? Are all months evenly represented by the dataset?
SELECT DATE_PART('month',occurred_at), SUM(total) total_qty
FROM orders
WHERE occurred_at BETWEEN '2014-01-01' AND '2017-01-01'
GROUP BY 1
ORDER BY 2 DESC;
...
SELECT a.name, DATE_TRUNC('month',occurred_at), SUM(gloss_amt_usd) gloss_total
FROM orders o
JOIN accounts a
ON o.account_id = a.id
...
SELECT CASE WHEN total >= 2000 THEN 'At Least 2000'
            ...
            ELSE 'Less Than 1000' END AS Type, COUNT(*)
FROM orders
GROUP BY 1;
...
SELECT a.name, SUM(o.total_amt_usd),
	CASE WHEN SUM(o.total_amt_usd) > 200000 THEN 'Greater than $200,000'
    	WHEN SUM(o.total_amt_usd) <= 200000 AND SUM(o.total_amt_usd) >= 100000 THEN 'Between $100,000 and $200,000'
        ELSE 'Less than $100,000' END as Level
FROM orders o
JOIN accounts a
ON o.account_id = a.id
GROUP BY a.name
...
SELECT s.name, COUNT(o.*), SUM(o.total_amt_usd),
